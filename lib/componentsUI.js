...
ajax(null,null,'get','/info')
	.then(function(posts){
		...
		if(swap=body.$1('.component.list')){
			var htmls=posts.map(function(p){
				return '<h6 data-keywords="'+(p.keywords||'')+'"><a href="'+p.name+'.html">'+p.name+'</a></h6>'
			});
			htmls.push('<style>.component.list h6{white-space:nowrap;overflow:hidden}</style>')
			
			swap.innerHTML=htmls.join('')
		}
...
if(swap=body.$1('.component.search')){
	...
	swap.$1('input').onchange=function(){
		...
		list.childNodes.forEach(function(a){a.style.display='none'})
